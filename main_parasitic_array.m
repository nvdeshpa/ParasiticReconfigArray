...
for np_i = 1:length(Np_arr)
    for na_i=1:length(Na_arr)
        ...
        elseif impedance_mat_from_feko==1
            if Np==1
                Z_matrix=load('Z_matrix_Na4Np1_all_zero_loads.mat');
            elseif Np==2
                Z_matrix=load('Z_matrix_Na4Np2_all_zero_loads.mat');
            elseif Np==3
                Z_matrix=load('Z_matrix_Na4Np3_all_zero_loads.mat');
            elseif Np==4
                Z_matrix=load('Z_matrix_Na4Np4_all_zero_loads.mat');
            end
        ...
        for ti=1:num_realizations
            ...
            for ny=1:Ny
                ...
                for np=1:Np
                   ...
                   %phi_opt= -1+1/ha(ny)*(   zeta/2*hp_j.'*(zm_jj)  + hp_j(np)*zm_jj(np)  );
                   %Izr_decoupled(np, ny)=  -imag(Z_P(1,1))- cot(0.5*angle(phi_opt))/zeta;
            ...
            %Z_eff_star=real(Z_A)-2*real(Z_M'*(inv(Z_P+Z_R_star))')*real(Z_M)  + (Z_M'*(inv(Z_P+Z_R_star))'*real(Z_P)*inv(Z_P+Z_R_star)*Z_M);
            Z_eff_star=real(Z_A)-(Z_M'*(inv(Z_P+Z_R_star))')*real(Z_M) - real(Z_M.')*inv(Z_P+Z_R_star)*Z_M + (Z_M'*(inv(Z_P+Z_R_star))'*real(Z_P)*inv(Z_P+Z_R_star)*Z_M);

            h_eff_star=ha-Z_M.'*inv(Z_P+Z_R_star)*hp;

            Z_fully_active=[Z_A  Z_M.';  Z_M  Z_P];
            ...
            for p_i=1:length(P_t_arr)
                ...
                %SNR_MAMP_arr(ti, p_i)= G/sigma_sq*(i_A_star.'*h_eff_star)^2;
...
% figure
% 
% plot(P_t_arr_dBm, SE_MAMP_averaged, '-x', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'HRP-UPA (Proposed)')
% hold on
% plot(P_t_arr_dBm, SE_hybrid_ps_arr_averaged, '-v', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'HPS-UPA (benchmark)')
% plot(P_t_arr_dBm, SE_fully_active_averaged, '-o', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'FA-UPA (benchmark)')
% plot(P_t_arr_dBm, SE_only_active_averaged, '-pent', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'FA-ULA (benchmark)')
% legend
% xlabel('Transmit power P_{max} (in dBm)')
...
% 
% figure
% plot(P_t_arr_dBm, EE_MAMP_averaged, '-x', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'HRP-UPA (Proposed)')
% hold on
% plot(P_t_arr_dBm, EE_hybrid_PS_averaged, '-v', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'HPS-UPA (benchmark)')
% plot(P_t_arr_dBm, EE_fully_active_averaged, '-o', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'FA-UPA (benchmark)')
% plot(P_t_arr_dBm, EE_only_active_averaged, '-pent', 'Markersize', 9, 'LineWidth', 3, 'DisplayName', 'FA-ULA (benchmark)')
% legend
% xlabel('Transmit power P_{max} (in dBm)')
% ylabel('Energy efficiency (bps/Hz)')
% grid on
% title('Energy efficiency vs transmit power for different architectures')


figure
plot([0, squeeze(Np_arr)],[SE_only_active_averaged(1); squeeze(SE_MAMP_averaged)] )
...
for ni=1:NA
    for np=1:NP+1
        if np==1
            ...
            %F_PS_subconnected(ni, (ni-1)*(NP+1)+1:ni*(NP+1))=f_ps((ni-1)*(NP+1)+1:ni*(NP+1));
        end
    end
end
